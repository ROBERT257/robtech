...
export default function PaymentStep() {
  ...
  const { user } = useAuth();
  const [phone, setPhone] = useState((params?.phone as string) || user?.phone || '');
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.subtitle}>One-time activation fee. Secure payment via M‑Pesa. Trusted by millions in Kenya.</Text>
      ...
      <TouchableOpacity style={styles.cta} onPress={onPay} disabled={loading}>
        <Text style={styles.ctaText}>{loading ? 'Processing...' : 'Pay with M‑Pesa'}</Text>
      </TouchableOpacity>
      <View style={{ marginTop: 12 }}>
        <Text style={{ color: '#ccc', fontSize: 12 }}>Secure payment • Encrypted • No card required</Text>
      </View>
      <View style={styles.badgesRow}>
        <Text style={styles.badge}>Secure payment</Text>
        <Text style={styles.badge}>Trusted by Kenyan users</Text>
        <Text style={styles.badge}>M‑Pesa protected</Text>
      </View>
    </View>
  );
}
